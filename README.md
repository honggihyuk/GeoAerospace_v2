...
- **자연어 지도 제어(P4)** — 로컬 **Qwen3-8B(Ollama)** 백본. 명령 바/챗 → `/api/agent`(Ollama `/api/chat`, tools, think:true) → 도구 실행(`fly_to_place`·`select_satellite`·`toggle_layer`). **결정론적 의도 해석 그라운딩 레이어**(§4.5)로 8B 도구선택 변동성 보정 + **지오코딩**(도시 테이블 + Nominatim 폴백)으로 좌표 환각 제거
